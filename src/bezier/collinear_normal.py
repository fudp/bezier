...
def distance_patch(nodes1, nodes2):
    ...
    in Bernstein form. Once this has been done, a B |eacute| surface patch
    with control points :math:`D_{ij} = \left[\begin{array}{c}
    ...
    for i in six.moves.xrange(2 * degree1):
        # control_mesh[j, i] == d_{ij}
        min_k = max(0, 1 + i - degree1)
        max_k = min(i, degree1)
        for k in six.moves.xrange(min_k, max_k + 1):
            ell = i - k
...
def clip_collinear(nodes1, nodes2):
    ...
    for j in six.moves.xrange(size_mesh):
    ...
    s_min = DEFAULT_S_MIN
    s_max = DEFAULT_S_MAX
    for start_index in six.moves.xrange(-1, size_polygon - 1):
